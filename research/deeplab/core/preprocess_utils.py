...
def randomly_scale_image_and_label_panoptic(image,
                                            label=None,
                                            label_center=None,
                                            label_offset=None,
                                            scale=1.0):
  ...
  # No random scaling if scale == 1.
  if scale == 1.0:
    return image, label, label_center, label_offset
  image_shape = tf.shape(image)
  new_dim = tf.cast(
      tf.cast([image_shape[0], image_shape[1]], tf.float32) * scale,
      tf.int32)
  ...
  if label_center is not None:
    label_center = tf.image.resize(
        label_center,
        new_dim,
        method=get_label_resize_method(label_center),
        align_corners=True)

  if label_offset is not None:
    label_offset = tf.image.resize(
        label_offset,
        new_dim,
        method=get_label_resize_method(label_offset),
        align_corners=True)
...
def resize_to_range_panoptic(image,
                            label=None,
                            label_center=None,
                            label_offset=None,
                            min_size=None,
  ...
  with tf.name_scope(scope, 'resize_to_range', [image]):
    ...
    if label_center is not None:
      if label_layout_is_chw:
        # Input label has shape [channel, height, width].
        resized_label = tf.expand_dims(label_center, 3)
        resized_label = tf.image.resize(
            resized_label,
            new_size,
            method=get_label_resize_method(label_center),
    ...
    if label_offset is not None:
      if label_layout_is_chw:
        # Input label has shape [channel, height, width].
        resized_label = tf.expand_dims(label_offset, 3)
        resized_label = tf.image.resize(
            resized_label,
            new_size,
            method=get_label_resize_method(label_offset),
