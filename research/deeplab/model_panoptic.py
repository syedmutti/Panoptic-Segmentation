...
LOGITS_SCOPE_NAME_INSTANCE = 'logits_instance_center'
LOGITS_SCOPE_NAME_OFFSET = 'logits_center_offset'
MERGED_LOGITS_SCOPE = 'merged_logits'
IMAGE_POOLING_SCOPE = 'image_pooling'
IMAGE_POOLING_SCOPE_INSTANCE = 'image_pooling_instance'
ASPP_SCOPE = 'aspp'
ASPP_SCOPE_INSTANCE = 'aspp_instance'
CONCAT_PROJECTION_SCOPE = 'concat_projection'
CONCAT_PROJECTION_SCOPE_INSTANCE = 'concat_projection_instance'
DECODER_SCOPE = 'decoder'
DECODER_SCOPE_INSTANCE = 'decoder_instance'
...
def predict_labels(images, model_options, image_pyramid=None):
  ...
  for output in sorted(outputs_to_scales_to_logits):
      if output == common.OUTPUT_TYPE:
      ...
      elif output is common.INSTANCE:
          ...
          if model_options.prediction_with_upsampled_logits:
              ...
              predictions[output + PROB_SUFFIX] = logits

          else:
              #argmax_results = tf.argmax(logits, 3)
              argmax_results = logits
              argmax_results = tf.image.resize_nearest_neighbor(
                  tf.expand_dims(argmax_results, 3),
                  tf.shape(images)[1:3],
                  align_corners=True,
...
def extract_features(images,
  ...
  if not model_options.aspp_with_batch_norm:
    return features, features_instance, end_points
  else:
    if model_options.dense_prediction_cell_config is not None:
      ...
      concat_logits_instance = dense_prediction_layer.build_cell(
          features,
          output_stride=model_options.output_stride,
          crop_size=model_options.crop_size,
          image_pooling_crop_size=model_options.image_pooling_crop_size,
      ...
      return concat_logits, concat_logits_instance, end_points
    else:
      ...
      with slim.arg_scope(
        ...
        with slim.arg_scope([batch_norm], **batch_norm_params):
          ...
          ##############################################################################
          cancat_logits_instance = cancat_logits_aspp_second(images,
                                    features,
                                    model_options,
                                    weight_decay=weight_decay,
                                    reuse=reuse,
          ...
          return concat_logits, cancat_logits_instance, end_points

def cancat_logits_aspp_second(images,
                             features,
                             model_options,
                             weight_decay=0.0001,
                             reuse=None,
        ...
        with slim.arg_scope([batch_norm], **batch_norm_params):
            ...
            if model_options.add_image_level_feature:
                ...
                image_feature = slim.conv2d(
                    ...
                    activation_fn=image_feature_activation_fn,
                    normalizer_fn=image_feature_normalizer_fn,
                    scope=IMAGE_POOLING_SCOPE_INSTANCE)
                image_feature = _resize_bilinear(
                    image_feature,
                    [resize_height, resize_width],
                    image_feature.dtype)
            ...
            # Employ a 1x1 convolution.
            branch_logits.append(slim.conv2d(features, depth, 1,
                                             scope=ASPP_SCOPE_INSTANCE + str(0)))

            if model_options.atrous_rates:
                # Employ 3x3 convolutions with different atrous rates.
                for i, rate in enumerate(model_options.atrous_rates, 1):
                    scope = ASPP_SCOPE_INSTANCE + str(i)
                    if model_options.aspp_with_separable_conv:
                        aspp_features = split_separable_conv2d(
                            features,
                            filters=depth,
            ...
            if model_options.aspp_with_concat_projection:
                concat_logits = slim.conv2d(
                    concat_logits, depth, 1, scope=CONCAT_PROJECTION_SCOPE_INSTANCE)
                concat_logits = slim.dropout(
                    concat_logits,
                    keep_prob=0.9,
                    is_training=is_training,
                    scope=CONCAT_PROJECTION_SCOPE_INSTANCE + '_dropout')
            if (model_options.add_image_level_feature and
                    model_options.aspp_with_squeeze_and_excitation):
                concat_logits *= image_feature
...
def _get_logits(images,
  ...
  ##### Extract features and endpoints from Latent Space ####
  features_semantic, features_instance, end_points = extract_features(
      images,
      model_options,
      weight_decay=weight_decay,
      reuse=reuse,
  ...
  if model_options.decoder_output_stride:
    ...
    features_semantic = refine_by_decoder(
        features_semantic,
        end_points,
        crop_size=crop_size,
    ...
    features_instance = refine_by_decoder_instance(
                          features_instance,
                          end_points,
                          crop_size=crop_size,
                          decoder_output_stride=model_options.decoder_output_stride,
                          decoder_use_separable_conv=model_options.decoder_use_separable_conv,
                          decoder_use_sum_merge=model_options.decoder_use_sum_merge,
                          decoder_filters=model_options.decoder_filters_instance,
                          decoder_output_is_logits=model_options.decoder_output_is_logits,
                          model_variant=model_options.model_variant,
                          weight_decay=weight_decay,
                          reuse=reuse,
  ...
  for output in sorted(model_options.outputs_to_num_classes):
    if model_options.decoder_output_is_logits:
        if output is common.OUTPUT_TYPE:
          outputs_to_logits[output] = tf.identity(features_semantic,
                                              name=output)
        elif output is common.INSTANCE:
            outputs_to_logits[output] = tf.identity(features_instance,
                                                    name=output)
        elif output is common.OFFSET:
            outputs_to_logits[output] = tf.identity(features_instance,
                                                    name=output)
       ...
       if output is common.OUTPUT_TYPE:
          outputs_to_logits[output] = get_branch_logits(
           ...
           outputs_to_logits[output] = get_branch_logits_instance_center(
               features_instance,
               model_options.outputs_to_num_classes[output],
               model_options.atrous_rates,
               aspp_with_batch_norm=model_options.aspp_with_batch_norm,
           ...
           outputs_to_logits[output] = get_branch_logits_instance_offset(
               features_instance,
               model_options.outputs_to_num_classes[output],
               model_options.atrous_rates,
               aspp_with_batch_norm=model_options.aspp_with_batch_norm,
  ...
  with slim.arg_scope(
    ...
    with slim.arg_scope([batch_norm], **batch_norm_params):
      with tf.variable_scope(DECODER_SCOPE, DECODER_SCOPE, [features]):
        ...
        decoder_stage = 0
        scope_suffix = ''
        for index, output_stride in enumerate(decoder_output_stride):
...
def refine_by_decoder_instance(features,
                              end_points,
                              crop_size=None,
                              decoder_output_stride=None,
                              decoder_use_separable_conv=False,
  ...
  Args:
    ...
    decoder_use_separable_conv: Employ separable convolution for decoder or not.
    decoder_use_sum_merge: Boolean, decoder uses simple sum merge or not.
    decoder_filters: Integer array, decoder filter size.
    decoder_output_is_logits: Boolean, using decoder output as logits or not.
    model_variant: Model variant for feature extraction.
    weight_decay: The weight decay for model variables.
    reuse: Reuse the model variables or not.
  ...
  with slim.arg_scope(
      ...
      reuse=reuse):
    with slim.arg_scope([batch_norm], **batch_norm_params):
      with tf.variable_scope(DECODER_SCOPE_INSTANCE, DECODER_SCOPE_INSTANCE, [features]):
        decoder_features = features
        decoder_stage = 0
        scope_suffix = ''
        for index, output_stride in enumerate(decoder_output_stride):
          ...
          # order for backward compactibility.
          if decoder_stage:
            scope_suffix = '_{}_INSTANCE'.format(decoder_stage)
          for i, name in enumerate(feature_list):

            #### feature_list = 8 and 4
            ...
            if decoder_use_sum_merge:
              ...
              if not decoder_use_separable_conv:
                scope_suffix = str(i) + scope_suffix
              decoder_features = _decoder_with_concat_merge_instance(
                  decoder_features_list,
                  decoder_depth,
                  decoder_use_separable_conv=decoder_use_separable_conv,
                  weight_decay=weight_decay,
...
def _decoder_with_concat_merge_instance(decoder_features_list,
                                       decoder_depth,
                                       decoder_use_separable_conv=True,
                                       weight_decay=0.0001,
                                       scope_suffix=''):
...
def get_branch_logits(features,
  ...
  with slim.arg_scope(
    ...
    with tf.variable_scope(LOGITS_SCOPE_NAME, LOGITS_SCOPE_NAME, [features]):
      ...
      features = split_separable_conv2d(
          features,
          filters=256,
          kernel_size=5,
          rate=1,
          weight_decay=weight_decay,
          scope='branched_logits_semantic_conv0' + scope_suffix)
...
def get_branch_logits_instance_center(features,
                                      num_classes,
                                      atrous_rates=None,
                                      aspp_with_batch_norm=False,
                                      kernel_size=1,
  ...
  with slim.arg_scope(
    ...
    with tf.variable_scope(LOGITS_SCOPE_NAME_INSTANCE, LOGITS_SCOPE_NAME_INSTANCE, [features]):
      branch_logits = []
      ...
      features = split_separable_conv2d(
          ...
          rate=1,
          weight_decay=weight_decay,
          scope='branched_logit_instance_conv0' + scope_suffix)
...
def get_branch_logits_instance_offset(features,
                      num_classes,
                      atrous_rates=None,
                      aspp_with_batch_norm=False,
                      kernel_size=1,
  ...
  with slim.arg_scope(
      ...
      weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
      reuse=reuse):
    with tf.variable_scope(LOGITS_SCOPE_NAME_OFFSET, LOGITS_SCOPE_NAME_OFFSET, [features]):
      branch_logits = []
      ...
      features = split_separable_conv2d(
          ...
          rate=1,
          weight_decay=weight_decay,
          scope='branched_logit_offset_conv0' + scope_suffix)
