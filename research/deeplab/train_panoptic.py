...
def _build_deeplab(iterator, outputs_to_num_classes, ignore_label):
  ...
  samples[common.LABEL_INSTANCE] = tf.identity(samples[common.LABEL_INSTANCE], name=common.LABEL_INSTANCE)
  samples[common.LABEL_OFFSET] = tf.identity(samples[common.LABEL_OFFSET], name=common.LABEL_OFFSET)
  ...
  output_type_dict_instance = outputs_to_scales_to_logits[common.INSTANCE]
  output_type_dict_instance[model.MERGED_LOGITS_SCOPE] = tf.identity(
      output_type_dict_instance[model.MERGED_LOGITS_SCOPE], name=common.INSTANCE)

  output_type_dict_offset = outputs_to_scales_to_logits[common.OFFSET]
  output_type_dict_offset[model.MERGED_LOGITS_SCOPE] = tf.identity(
      output_type_dict_offset[model.MERGED_LOGITS_SCOPE], name=common.OFFSET)
  ...
  for output, num_classes in six.iteritems(outputs_to_num_classes):
    ...
    elif output == common.INSTANCE:
        train_utils.add_heatmap_regression(
            outputs_to_scales_to_logits[output],
            samples[common.LABEL_INSTANCE],
            upsample_logits=FLAGS.upsample_logits,
            top_k_percent_pixels=FLAGS.top_k_percent_pixels,
            scope=output)

    elif output == common.OFFSET:
        train_utils.add_l1_loss_to_each_dimension(
            outputs_to_scales_to_logits[output],
            samples[common.LABEL_OFFSET],
            upsample_logits=FLAGS.upsample_logits,
            top_k_percent_pixels=FLAGS.top_k_percent_pixels,
            scope=output)
...
def main(unused_argv):
  ...
  with tf.Graph().as_default() as graph:
    ...
    if FLAGS.save_summaries_images:
      ...
      first_clone_label = tf.multiply(tf.divide(first_clone_label, label_max_x), 255.0)
      ...
      summaries.add(
          tf.summary.image('samples/%s' % common.INSTANCE, summary_label))

      ########## INSTANCE OFFSET ###########

      first_clone_label = graph.get_tensor_by_name(
          ('%s/%s:0' % (first_clone_scope, common.LABEL_OFFSET)).strip('/'))
      ...
      first_clone_label = tf.concat([x_offset, y_offset, channel_padding], 3)

      summary_label = tf.image.convert_image_dtype(first_clone_label, dtype=tf.uint8, saturate=True)
      summaries.add(
          tf.summary.image('samples/%s' % common.LABEL_OFFSET, summary_label))
