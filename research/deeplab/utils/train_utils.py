...
def add_heatmap_regression(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    with tf.name_scope(loss_scope, default_loss_scope,
      ...
      center_loss = tf.squared_difference(logits, tf.stop_gradient(scaled_labels))
      center_loss = tf.reduce_mean(center_loss, 0)      # Across batach mean
      num_present = tf.reduce_sum(tf.cast(tf.not_equal(center_loss, 0), tf.float32))
      loss = tf.reduce_sum(center_loss)      # Sum everything
...
def add_l1_loss_to_each_dimension(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    with tf.name_scope(loss_scope, default_loss_scope,
      ...
      center_loss = tf.abs(tf.subtract(logits, tf.stop_gradient(scaled_labels)))
