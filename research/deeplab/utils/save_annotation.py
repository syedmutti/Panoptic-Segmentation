...
def save_annotation_heatmaps(label,
                    save_dir,
                    filename,
                    add_colormap=True,
...
def save_annotation_offset_vectors(label,
                    save_dir,
                    filename):
  """Saves the given label to image on disk."""
...
def save_annotation_overlayed(label,
                    original_image,
                    save_dir,
                    filename,
                    add_colormap=True,
  ...
  pil_image = img.blend(img.fromarray(original_image.astype(dtype=np.uint8)), img.fromarray(colored_label.astype(dtype=np.uint8)), 0.5)
  with tf.gfile.Open('%s/%s.png' % (save_dir, filename), mode='w') as f:
    pil_image.save(f, 'PNG')

def save_annotation_panoptic(label,
                            save_dir,
                            filename,
                            add_colormap=True,
                            normalize_to_unit_values=False,
...
def save_annotation_instance_segmentation(label,
                                        save_dir,
                                        filename,
                                        add_colormap=False,
  ...
  if add_colormap:
    ...
    if normalize_values:
      ...
      #range_value = max_value - min_value
      if max_value != 0:
        colored_label = (colored_label / max_value) * 255.0
